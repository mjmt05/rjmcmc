#ifndef HISTOGRAM_TYPE_HPP
#define HISTOGRAM_TYPE_HPP
...
class Histogram_Type : public Histogram
{
 public:
  ...
  ~Histogram_Type();
  void set_value_function(double (T::*get_component_value)() const){ m_get_component_value = get_component_value; }
 ...
 protected:
  double (T::*m_get_component_value)() const;
...
Histogram_Type<T>::Histogram_Type(double start, double end, unsigned int num_bins, double bin_width, bool bounded, bool weighted, bool one_d,unsigned int max_dim, string* input_filename, bool calculate_divergence, Divergence_Type divergence_type, Loss_Function loss_fn, unsigned int look_up_length,bool estimate_autocorrelation):Histogram(start,end,num_bins,bin_width,bounded,weighted,one_d,max_dim,input_filename,calculate_divergence,divergence_type,loss_fn,look_up_length,estimate_autocorrelation){
  m_get_component_value = NULL;//&double_dereference;//NULL;
...
void Histogram_Type<T>::calculate_bin( Particle<T>* particle, unsigned int particle_dim ){
  ...
  if(!m_use_single_component){
	  ...
	  double val;
	  if(m_get_component_value){
	    val = (component_k->*m_get_component_value)();//m_get_component_value? m_get_component_value( component_k ): static_cast<double>(*component_k);
	  }else{
	    val=double_dereference(component_k);
	  }
    ...
    if(m_use_single_component==2){
      T* component = particle->get_theta_component(particle_dim-1);
      if(m_get_component_value){
	Histogram::calculate_bin( (component->*m_get_component_value)() );
      }else{
	Histogram::calculate_bin(double_dereference(component));
      }
    }else if(m_use_single_component==1 || m_use_single_component==3){
      if(m_get_component_value){
	Histogram::calculate_bin( (particle->get_theta_component(0)->*m_get_component_value)());
      }else{
	Histogram::calculate_bin( double_dereference(particle->get_theta_component(0)));
      }
    }
    if(m_use_single_component==3 && particle_dim>1){
      if(m_get_component_value){
	Histogram::calculate_bin( (particle->get_theta_component(particle_dim-1)->*m_get_component_value)() );
      }else{
	Histogram::calculate_bin(double_dereference(particle->get_theta_component(particle_dim-1)) );
      }
    }
	...
	for(unsigned int k=0; k<particle_dim; k++){
          T* component_k = particle->get_theta_component(k);
          double val = (component_k->*m_get_component_value)();//m_get_component_value? m_get_component_value( co
